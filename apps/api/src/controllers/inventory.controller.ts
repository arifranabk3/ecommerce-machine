...
export class InventoryController {
  static async adjustStock(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const result = await InventoryService.adjustStock(tenantId, { ...input, warehouseId: input.warehouseId }, actorUserId);
  ...
  static async transferStock(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const result = await InventoryService.transferStock(tenantId, { ...input, fromWarehouseId: input.fromWarehouseId, toWarehouseId: input.toWarehouseId }, actorUserId);
      return res.json({ success: true, data: result });
    } catch (err) {
      next(err);
    }
  ...
  static async reserveStock(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      ...
      const result = await InventoryService.reserveStock(tenantId, { ...input, warehouseId: input.warehouseId }, actorUserId);
