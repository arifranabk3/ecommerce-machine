...
export default function VendorProductsPage() {
  ...
  const { data: productsData, isLoading } = useApiQuery<any>(`/api/v1/products?vendorId=${params.id}`);
  const products = productsData?.items || productsData?.data?.items || productsData?.data || productsData || [];
                ...
                {products.length === 0 ? (
                  ...
                  products.map((product: any) => (
                    <tr key={product._id} className="hover:bg-gray-50/50 transition">
                      ...
                      <td className="px-6 py-4 font-medium">PKR {product.costPriceMinor ? (product.costPriceMinor / 100).toFixed(2) : '0.00'}</td>
                      <td className="px-6 py-4">PKR {product.sellingPriceMinor ? (product.sellingPriceMinor / 100).toFixed(2) : '0.00'}</td>
                      <td className="px-6 py-4">
